...
contract TranscriptAttendance is AccessControl {
    ...
    // --- teacher
    function issueGrade(address student, uint256 courseId, string calldata grade, string calldata ipfsHash)
    ...
    function markAttendance(address student, uint256 courseId, uint256 date, Status status, string calldata ipfsHash)
        external onlyTeacher
    {
        Course memory c = courses[courseId];
        if (!c.exists) revert CourseNotFound();
    ...
    function verifyTranscript(address student, uint256 courseId)
        external payable returns (GradeRecord memory rec)
    {
    ...
    // --- views
    function viewRecord(address student, uint256 courseId) external view returns (GradeRecord memory rec) {
        rec = records[_key(student, courseId)];
    }

    function viewAttendance(address student, uint256 courseId) external view returns (AttendanceEntry[] memory) {
